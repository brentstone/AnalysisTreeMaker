...
def jetToolbox( proc, jetType, jetSequence, outputFile,
	...
	if not updateCollection: 
		...
		if 'Puppi' in PUMethod:
			...
			setattr( proc, jetalgo+'PFJets'+ PUMethod, 
					ak4PFJetsPuppi.clone( src = cms.InputTag( srcForPFJets ),
						doAreaFastjet = True, 
						rParam = jetSize, 
						jetAlgorithm = algorithm ) )  
			jetSeq += getattr(proc, jetalgo+'PFJets'+PUMethod )
		...
		if ( PUMethod in [ 'CHS', 'CS', 'Puppi', 'PuppiNoLep' ] ) and miniAOD: setattr( proc, jetalgo+'PFJets'+PUMethod+'Constituents', cms.EDFilter("MiniAODJetConstituentSelector", src = cms.InputTag( jetalgo+'PFJets'+PUMethod ), cut = cms.string( Cut ) ))
