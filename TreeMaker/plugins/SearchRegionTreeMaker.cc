	...
	SearchRegionTreeMaker(const edm::ParameterSet &cfg) : AnalysisTreeMaker(cfg)
{
		...
		initialize(new AnaTM::ElectronFiller(cfg,"ElectronFiller",consumesCollector(),getDataEra(),(const AnaTM::EventFiller*)eventFiller));
		initialize(new AnaTM::MuonFiller(cfg,"MuonFiller",consumesCollector(),isRealData(),getDataEra(),(const AnaTM::EventFiller*)eventFiller));
		if(!isRealData())
